...
class DBProcessor(BaseProcessor):
    ...
    def find_dbf_files(root_dir):
        """Yield DBF file paths in the given directory."""
        return (Path(root) / file for root, _, files in os.walk(root_dir) for file in files if
                file.lower().endswith(".dbf"))

    def process_table_records(self, table, entries):
        """Process records within a table."""
        for i, record in enumerate(table):
            outputs = [record[rec[0]] for rec in entries]
            print(outputs)
            # Placeholder for further processing logic

    def process_dbf_file(self, file_path, keys):
        """Process a single DBF file and check for COMMENT fields."""
        if not file_path.exists() or not file_path.suffix.lower() == ".dbf":
            return False  # Early return if not a DBF file
        ...
        try:
            ...
            entries = [
                (field, table.field_info(field)[1])
                for field in table.field_names
                if field in keys
            ]
            if not any(entry[0] in {"COMMENT", "_COMMENT"} for entry in entries):
                return False  # Early return if no COMMENT fields

            self.logger.log(f"Table {file_path.name} contains COMMENT or _COMMENT field.", log_level=1)
            self.process_table_records(table, entries)
            return True
        except Exception as e:
            self.logger.log(f"Error processing table {file_path}: {e}", log_level=2)
        finally:
            if 'table' in locals():
                table.close()
        return False
    ...
    async def get_tables_with_comment_field(self, root_dir):
        """Search for DBF files with COMMENT fields in a directory."""
        self.logger.log(f"Searching for COMMENT fields in {root_dir}")
        cnt = 0
        keys = ["_SKZ", "_UNAME", "NAME", "LANGNAME", "_COMMENT", "COMMENT"]

        for file_path in self.find_dbf_files(root_dir):
            if self.process_dbf_file(file_path, keys):
                cnt += 1
        ...
        #                 if any(
        #                     entry[0] in {"COMMENT", "_COMMENT"} for entry in entries
        #                 ):
        #                     cnt += 1
        #                     self.logger.log(
        #                         f"Table {file} contains COMMENT or _COMMENT field.",
        #                         log_level=1,
        #                     )
        #                     print(entries)
        #                     for i, record in enumerate(table):
...
class TranslationApp(toga.App):
    ...
    def startup(self):
        ...
        self.dropdown = toga.Selection(
            items=list(self.encodings_kv_swapped.keys()), style=Pack(padding=5)
        )
        self.dropdown.value = next(
            (v for k, v in conf["code_pages"].items() if k == conf["default_encoding"]),
            None,
        )
        self.main_box.add(self.dropdown)
        self.dropdown.on_change = lambda widget: conf.__setitem__(
            "default_encoding", self.encodings_kv_swapped[widget.value]
        )

        # Add buttons
        self.menu_box = toga.Box(style=Pack(direction=ROW, padding=10))
        self.select_db_button = toga.Button(
            conf["tr"][self.lang]["select_db_dir"], on_press=self.select_db_directory
        )
        self.process_db_button = toga.Button(
            conf["tr"][self.lang]["process_db"], on_press=self.process_databases
        )
    ...
    async def select_db_directory(self, widget):
        ...
        selected_dir = await self.main_window.select_folder_dialog(
            title=conf["tr"][self.lang]["select_db_dir"]
        )
        if selected_dir:
            self.db_root_dir = Path(selected_dir)
            self.logger.log(f"{conf['tr'][self.lang]['selected_dir']}  {selected_dir}",log_level=1)
    async def process_databases(self, widget):
        ...
        db_processor = DBProcessor(
            self.db_root_dir, self.translator, self.logger, self.encoding
        )
